...
class OperatorsTrustOperation:
    def __init__(self, adb_path="D:\\Program Files\\Netease\\MuMuPlayer-12.0\\shell\\adb.exe", adb_port="16384"):
        ...
        self._setup_logging()
        self.target_img_path = os.path.join(os.path.dirname(__file__),'..', '..', 'screenshots', 'raw_screenshots', 'operators_trust.png')
        ...
        self.whether_operators_trust = 0
        ...
        while True:
            ...
            if self.monitoring.img is not None:
                ...
                if loc[0].size > 0:
                    ...
                    print(f"找到信赖收取，位置: {center}")
                    self.logger.info(f"找到信赖收取图标，位置: {center}")
                    mumu_adb.click(center[0], center[1])
                    print(f"已经收取干员信赖，中心位置位置在{center}")
                    break  # 退出循环
                # 如果未找到，重复尝试5次后退出循环
                else:
                    self.whether_operators_trust += 1
                    print(f"也许已经收取过了，尝试次数 {self.whether_operators_trust}")
                    if self.whether_operators_trust >= 5:
                        break
