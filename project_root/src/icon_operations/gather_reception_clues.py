...
class GatherReceptionCluesOperation:
    def __init__(self, adb_path="D:\\Program Files\\Netease\\MuMuPlayer-12.0\\shell\\adb.exe", adb_port="16384"):
        ...
        self._setup_logging()
        self.target_img_path = os.path.join(os.path.dirname(__file__),'..', '..', 'screenshots', 'raw_screenshots', 'gather_reception_clues.png')
        ...
        self.whether_gather_reception_clues = 0
        ...
        while True:
            ...
            if self.monitoring.img is not None:
                ...
                if loc[0].size > 0:
                    ...
                    print(f"已然可以获得会客室线索，位置: {center}")
                    self.logger.info(f"找到获得会客室线索界面入口，位置: {center}")
                    mumu_adb.click(center[0], center[1])
                    print(f"成功领取会客室线索，中心位置位置在{center}")
                    break  # 退出循环
                # 如果未找到，重复尝试5次后退出循环
                else:
                    self.whether_gather_reception_clues += 1
                    print(f"貌似不能领取会客室线索，尝试次数 {self.whether_gather_reception_clues}")
                    if self.whether_gather_reception_clues >= 5:
                        break
