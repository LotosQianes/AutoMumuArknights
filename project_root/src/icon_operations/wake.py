...
class WakeOperation:
    def __init__(self, adb_path="D:\\Program Files\\Netease\\MuMuPlayer-12.0\\shell\\adb.exe", adb_port="16384"):
        ...
        self._setup_logging()
        self.target_img_path = os.path.join(os.path.dirname(__file__),'..', '..', 'screenshots', 'raw_screenshots', 'wake.png')
        ...
        while True:
            ...
            if self.monitoring.img is not None:
                ...
                if loc[0].size > 0:
                    ...
                    print(f"找到唤醒图标，位置: {center}")
                    self.logger.info(f"找到唤醒图像，位置: {center}")
                    mumu_adb.click(center[0], center[1])
                    print(f"已点击唤醒图标，中心位置位置在{center}，Welcome Back, Doctor.")
                    break  # 退出循环
                # 如果未找到，重复尝试20次后退出循环
                else:
                    self.whether_wake += 1
                    print(f"目前未找到唤醒图标，尝试唤醒次数 {self.whether_wake}")
                    if self.whether_wake >= 20:
                        break
