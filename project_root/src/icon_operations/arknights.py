        ...
        while True:
            ...
            if self.monitoring.img is not None:
                ...
                if loc[0].size > 0:
                    ...
                    self.logger.info(f"找到目标图像，位置: {center}")
                    mumu_adb.click(center[0], center[1])
                    print(f"已点击Arknights图标，中心位置位置在{center}")
                    break  # 退出循环
                # 如果未找到，重复尝试5次后退出循环
                else:
                    self.whether_arknights += 1
                    print(f"目前未找到图像，尝试次数 {self.whether_arknights}")
                    if self.whether_arknights >= 5:
                        break
