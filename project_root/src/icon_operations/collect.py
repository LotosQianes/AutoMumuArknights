...
class CollectOperation:
    def __init__(self, adb_path="D:\\Program Files\\Netease\\MuMuPlayer-12.0\\shell\\adb.exe", adb_port="16384"):
        ...
        self._setup_logging()
        self.target_img_path = os.path.join(os.path.dirname(__file__),'..', '..', 'screenshots', 'raw_screenshots', 'collect.png')
        ...
        while True:
            ...
            if self.monitoring.img is not None:
                ...
                if loc[0].size > 0:
                    ...
                    print(f"正在领取，位置: {center}")
                    self.logger.info(f"找到领取图标，位置: {center}")
                    mumu_adb.click(center[0], center[1])
                    print(f"成功领取，中心位置位置在{center}")
                    break  # 退出循环
                # 如果未找到，重复尝试5次后退出循环
                else:
                    self.whether_collect += 1
                    print(f"识别不到领取图标，尝试次数 {self.whether_collect}")
                    if self.whether_collect >= 5:
                        break
