...
class ReceivingMailOperation:
    def __init__(self, adb_path="D:\\Program Files\\Netease\\MuMuPlayer-12.0\\shell\\adb.exe", adb_port="16384"):
        ...
        self._setup_logging()
        self.target_img_path = os.path.join(os.path.dirname(__file__),'..', '..', 'screenshots', 'raw_screenshots', 'receiving_mail.png')
        ...
        while True:
            ...
            if self.monitoring.img is not None:
                ...
                if loc[0].size > 0:
                    ...
                    print(f"找到收取邮件图标，位置: {center}")
                    self.logger.info(f"找到收取邮件图像，位置: {center}")
                    mumu_adb.click(center[0], center[1])
                    print(f"您已收取邮件，中心位置位置在{center}")
                    break  # 退出循环
                # 如果未找到，重复尝试5次后退出循环
                else:
                    self.whether_receiving_mail += 1
                    print(f"目前未找到收取邮件图标，尝试次数 {self.whether_receiving_mail}")
                    if self.whether_receiving_mail >= 5:
                        break
