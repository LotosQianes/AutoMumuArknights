...
class InfrastructureTipsOperation:
    def __init__(self, adb_path="D:\\Program Files\\Netease\\MuMuPlayer-12.0\\shell\\adb.exe", adb_port="16384"):
        ...
        self._setup_logging()
        self.target_img_path = os.path.join(os.path.dirname(__file__),'..', '..', 'screenshots', 'raw_screenshots', 'infrastructure_tips.png')
        ...
        self.whether_infrastructure_tips = 0
        ...
        while True:
            ...
            if self.monitoring.img is not None:
                ...
                if loc[0].size > 0:
                    ...
                    print(f"找到基建提示，位置: {center}")
                    self.logger.info(f"找到基建图标，位置: {center}")
                    mumu_adb.click(center[0], center[1])
                    print(f"准备收货，中心位置位置在{center}")
                    break  # 退出循环
                # 如果未找到，重复尝试5次后退出循环
                else:
                    self.whether_infrastructure_tips += 1
                    print(f"尚未找到基建提示，尝试次数 {self.whether_infrastructure_tips}")
                    if self.whether_infrastructure_tips >= 5:
                        break
