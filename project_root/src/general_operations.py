...
class GeneralOperations:
	...
	def gather_infrastructure_mats(self):
		"""启动 '收集基建材料（信赖）' 功能"""
		try:
			self.logger.info("收集基建材料（信赖）功能启动")
			...
			self.logger.error(f"收集基建材料（信赖）功能发生错误: {e}")

	def gather_infrastructure_hint(self):
		"""启动 '收集基建线索' 功能"""
		try:
			self.logger.info("收集基建线索功能启动")
			...
			self.logger.error(f"收集基建线索功能发生错误： {e}")